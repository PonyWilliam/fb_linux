...
var (
	...
	appid     = "e75c3b68"
	apiSecret = "MTVkYTdjZDFmNmRmZGE1ZTA0NWUyMjQx"
    apiKey    = "4a4d93ec921849db32375e8b266362ee"
    file      = "1.pcm" //请填写您的音频文件路径
    conn *websocket.Conn
    resp *http.Response
    err2 error
    Status int64
...
func Listen() {
    ...
    //握手并建立websocket 连接
    conn, resp, err2 = d.Dial(assembleAuthUrl(hostUrl, apiKey, apiSecret), nil)
    if err2 != nil {
        panic(readResp(resp)+err2.Error())
        return
    ...
    for {
        ...
        if strings.Contains(resp.Data.Result.String(),"上一"){
            Code = 0
        }else if strings.Contains(resp.Data.Result.String(),"下一"){
            Code = 1
        }else if strings.Contains(resp.Data.Result.String(),"关灯"){
            Code = 2
        }else if strings.Contains(resp.Data.Result.String(),"开灯"){
            Code = 3
        }else if strings.Contains(resp.Data.Result.String(),"确认重启"){
            Code = 4
        }else if strings.Contains(resp.Data.Result.String(),"暂停"){
            Code = 5
